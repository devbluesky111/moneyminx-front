...
export type PlaceholderType = 'chart' | 'investment' | 'other' | 'liabilities' | 'networth' | 'manual' | 'acctDetail' | 'syncError';
...
export const Placeholder: React.FC<PlaceholderProps> = ({ type }) => {
    ...
    useEffect(() => {
        const messagesObject = {
            ...
            syncError: 'We are still processing this account. Check back soon.'
        }
        let message = '';
        switch (type) {
            case 'chart':
            ...
            case 'acctDetail':
                ...
                break;
            case 'syncError':
                message = messagesObject.syncError;
        }
        setMessage(message)
    }, [type]);
    ...
    return (
        <div className='networth-placeholder'>
            <p>{message}</p>
            {(type !== 'acctDetail' && type !== 'syncError') ?
                <Link to='/connect-account' className='mm-btn-animate mm-btn-primary'>
                    Add an account
            </Link> : null}
            {(type === 'syncError') ?
                <Link to='/net-worth' className='mm-btn-animate mm-btn-primary'>
                    Go to Dashboard
            </Link> : null}
        </div>
    )
