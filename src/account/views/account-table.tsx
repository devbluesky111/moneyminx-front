            ...
            {holdings?.length > 0 &&
              <div className="row no-gutters">
                ...
                {intervals?.length > 0 && intervals.map((item, index) => (
                  index < 2 ? (<div className='col-md mm-account-table__head--data d-xl-block' key={index}>{item}</div>) :
                    index > 6 ? (<div className='col-md mm-account-table__head--data d-xl-block' key={index}>{item}</div>) :
                      index === 4 ? (<div className='col-md mm-account-table__head--data d-md-block text-green' key={index}>{item}</div>) :
                        (<div className='col-md mm-account-table__head--data d-md-block' key={index}>{item}</div>)
                ))}
              </div>
            }
          </div>
            ...
            {holdings?.length > 0 && holdings.map((item, index) => (
              <div key={index}>
                ...
                <div className='row no-gutters mm-account-table__body--wrapper' onMouseEnter={() => { setEditIndex(index) }} onMouseLeave={() => { setEditIndex(-1) }}>
                  ...
                  <div className='col-4 col-md mm-account-table__body--data d-none d-xl-block'>${numberWithCommas(fNumber(item.price, 0))}</div>
                  ...
                  <div className='col-4 col-md mm-account-table__body--data d-none d-xl-block'><span className='d-block d-md-none'>{item.intervalValues[1].interval}</span>${item.intervalValues[1].value}</div>
                  ...
                  <div className='col-4 col-md mm-account-table__body--data'><span className='d-block d-md-none'>{item.intervalValues[5].interval}</span>${item.intervalValues[5].value}</div>
                  ...
                  <div className='col-4 col-md mm-account-table__body--data d-none d-xl-block'><span className='d-block d-md-none'>{item.intervalValues[7].interval}</span>${item.intervalValues[7].value}</div>
                  {editIndex === index ? <Edited className='edited-icon mt-2' /> : <></>}
                </div>
              </div>
            ))}
          </div>
          {(!holdings || holdings?.length === 0) && <span>No holdings data</span>}
        </div>
      </div>
    </section >
  );
